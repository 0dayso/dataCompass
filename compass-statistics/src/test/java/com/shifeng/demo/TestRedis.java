	...
	*//**
	...
	public static void main(String[] args) throws Exception {
		...
		for (int i=0;i<2000;i++)
		{
			...
			RedisTool.del("java framework");
	        System.out.println(RedisTool.lrange("java framework",0,-1));
	        //先向key java framework中存放三条数据  
	        RedisTool.lpush("java framework","spring");
	        RedisTool.lpush("java framework","struts");
	        RedisTool.lpush("java framework","hibernate");
	        RedisTool.lpush("java framework","struts");
	        RedisTool.lpush("java framework","spring");
	        //再取出所有数据RedisTool.lrange是按范围取出，
	        // 第一个是key，第二个是起始位置，第三个是结束位置，RedisTool.llen获取长度 -1表示取得所有
	        List<String> l = RedisTool.lrange("java framework",0,-1);
	        System.out.println(l);
	        RedisTool.lrem("java framework", 2, "struts");
	        System.out.println(RedisTool.lrange("java framework",0,-1));
	        RedisTool.del("java framework");
	       /* RedisTool.del("java framework");
	        RedisTool.rpush("java framework","spring");
	        RedisTool.rpush("java framework","struts");
	        RedisTool.rpush("java framework","hibernate");
	        System.out.println(RedisTool.lrange("java framework",0,-1));
	        System.out.println(RedisTool.lrange("java framework",0,0));
	        System.out.println(RedisTool.lrange("sms_yzm_key_18610347047",0,-1));
	       System.out.println(RedisTool.lrange("java framework",0,-1));
	       RedisTool.rpush(Const.CHECK_TICKET_QUEUE,"20160930110006195");
	       System.out.println(RedisTool.lrange(Const.CHECK_TICKET_QUEUE,0,-1));
		   ...
		   System.out.println((++i));
			...
			map.put("age", "12");
			// 存入一个map
			RedisTool.hmset("user", map); 
			
			// map key
			System.out.println("map的key" + RedisTool.hkeys("user")); 
			// (String key, String... fields)返回值是一个list
			List<String> list = RedisTool.hmget("user", "age", "name");
			System.out.println("redis中key的各个 fields值："
					+ RedisTool.hmget("user", "age", "name") + list.size());
			// 删除map中的某一个键 的值 password
			   
			System.out.println(RedisTool.hmget("user555", "name").get(0) == null); */
