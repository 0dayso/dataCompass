...
public class Purchase implements Serializable {
	 ...
	 * @param p
	 */
	public void combine(Visit p) {
		// 请求为空，将数据设置成缓存中的数据
		if (this.channelid == null) {
			this.channelid = p.getChannelid() == null ? 0 : p.getChannelid();
		} else {
			// 设置为最新的数据
			p.setChannelid(this.channelid);
		}

		if (StringUtils.isEmpty(this.channelUrlId)) {
			this.channelUrlId = StringUtils.isEmpty(p.getChannelUrlId()) ? "0" : p.getChannelUrlId();
		} else {
			p.setChannelUrlId(this.channelUrlId);
		}
