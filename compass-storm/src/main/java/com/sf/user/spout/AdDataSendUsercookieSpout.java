...
public class AdDataSendUsercookieSpout
  implements IRichSpout
{
  private static final long serialVersionUID = -6260803932233430770L;
  private static final Logger LOG = LoggerFactory.getLogger(AdDataSendUsercookieSpout.class);
  ...
  public void emit() {
    try {
      ...
      try { String adDataRedis = jedis.rpop(ConfKeys.STORM_AD_VISIT_DATA_COOKIE);
        if ((adDataRedis == null) || ("nil".equals(adDataRedis))) {
          ...
          Visit adVO = (Visit)JSONObject.toBean(adDataObject, Visit.class);
  ...
  {
    declarer.declare(new Fields(new String[] { "id", "Adcookie" }));
  }
