...
public class AdDataFollowShopSpout
  implements IRichSpout
{
  private static final long serialVersionUID = -6260803932233430770L;
  private static final Logger LOG = LoggerFactory.getLogger(AdDataFollowShopSpout.class);
  ...
  public void emit() {
    try {
      ...
      try { String adDataRedis = jedis.rpop(ConfKeys.STORM_AD_FOLLOW_SHOP_DATA);
        if ((adDataRedis == null) || ("nil".equals(adDataRedis))) {
          ...
          Shop adVO = (Shop)JSONObject.toBean(adDataObject, Shop.class);
  ...
  {
    declarer.declare(new Fields(new String[] { "id", "followshop" }));
  }
