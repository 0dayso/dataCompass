...
public class AdDataFollowSkuSpout
  implements IRichSpout
{
  private static final long serialVersionUID = -6260803932233430770L;
  private static final Logger LOG = LoggerFactory.getLogger(AdDataFollowSkuSpout.class);
  ...
  public void emit() {
    try {
      ...
      try { String adDataRedis = jedis.rpop(ConfKeys.STORM_AD_FOLLOW_SKU_DATA);
        if ((adDataRedis == null) || ("nil".equals(adDataRedis))) {
          ...
          Product adVO = (Product)JSONObject.toBean(adDataObject, Product.class);
  ...
  {
    declarer.declare(new Fields(new String[] { "id", "followsku" }));
  }
