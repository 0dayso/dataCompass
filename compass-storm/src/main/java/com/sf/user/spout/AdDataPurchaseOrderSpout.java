...
public class AdDataPurchaseOrderSpout
  implements IRichSpout
{
  private static final long serialVersionUID = -6260803932233430770L;
  private static final Logger LOG = LoggerFactory.getLogger(AdDataPurchaseOrderSpout.class);
  ...
  public void emit() {
    try {
      ...
      try { String adDataRedis = jedis.rpop(ConfKeys.STORM_AD_PURCHASE_ORDERID_DATA);
        if ((adDataRedis == null) || ("nil".equals(adDataRedis))) {
          ...
          Purchase adVO = (Purchase)JSONObject.toBean(adDataObject, Purchase.class);
  ...
  {
    declarer.declare(new Fields(new String[] { "id", "AdPurchaseOrder" }));
  }
