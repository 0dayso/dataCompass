...
public class AdDataSearchSpout
  implements IRichSpout
{
  private static final long serialVersionUID = -6260803932233430770L;
  private static final Logger LOG = LoggerFactory.getLogger(AdDataSearchSpout.class);
  ...
  public void emit() {
    try {
      ...
      try { String adDataRedis = jedis.rpop(ConfKeys.STORM_AD_SEARCH_DATA);
        if ((adDataRedis == null) || ("nil".equals(adDataRedis))) {
          ...
          Search adVO = (Search)JSONObject.toBean(adDataObject, Search.class);
  ...
  {
    declarer.declare(new Fields(new String[] { "id", "AdSearch" }));
  }
