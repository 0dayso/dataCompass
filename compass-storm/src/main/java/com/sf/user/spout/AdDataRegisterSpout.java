...
public class AdDataRegisterSpout
  implements IRichSpout
{
  private static final long serialVersionUID = -6260803932233430770L;
  private static final Logger LOG = LoggerFactory.getLogger(AdDataRegisterSpout.class);
  ...
  public void emit() {
    try {
      ...
      try { String adDataRedis = jedis.rpop(ConfKeys.STORM_AD_REGISTER_DATA);
        if ((adDataRedis == null) || ("nil".equals(adDataRedis))) {
          ...
          Register adVO = (Register)JSONObject.toBean(adDataObject, Register.class);
  ...
  {
    declarer.declare(new Fields(new String[] { "id", "AdRegister" }));
  }
