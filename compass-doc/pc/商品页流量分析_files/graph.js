...
/**
 * imgLine.origin: 10个来源线 imgLine.where: 5个去向线
 */
var imgLine = {
    origin: [{
        x: 241,
...
var rad = Math.PI / 180;
var originLine = '../../images/graph/arrow-come.gif';
var whereLine = '../../images/graph/arrow-go.gif';

// var dataOrigin = mixArray(staticOriginData, jsonObj.origin)
var dataWhere = staticWhereData;

/**
 * 将from数组中元素的属性都拷贝到对应的dest里去 var dest = [{name: "John"}, {name: "Backus"}] var
 * from = [{age: 30}, {age: 99}] mixArray(dest, from) // [{name: 'John', age:
 * 30}, {name: 'Backus', age: 99}]
 */
...
function subText(text,length){
	...
	if(text && text.length){
 		len = text.length;
		var half = (length+1)/2; 
		return (len > half) ? (text.substr(0,half)+"\n"+text.substr(half,length-half)+"..."):text;
...
GoodsGraph.prototype = {
    ...
    /**
	 * 商品 渲染函数 1) 中心点与底图一致 2) 半径，填充可在new时配置
	 */
    renderGoods: function() {
        var name = this.data.goods.name;
        var x = staticDoods.x;
        var y = staticDoods.y;
        this.circle(x, y, staticDoods.r, staticDoods.option);
        // 商品描述
        var txt = this.paper.text(x, y+5, subText(name,18));
        ...
        $(tspan).attr("desc", name);
        $(tspan).attr("class", "icon-help");
        $('.icon-help',$("#topTen")).Jtips({
            "content": "暂无描述",
            "position": 'bottom',
            "width": 250
        });
...
// {
// window.open(data.goods.goods_pic_src, "_blank" );
// });
		// this.paper.text(goods.name,x-25, y-35, 50,50);
		// var renderGoods_image = this.paper.image(data.goods.goods_src,x-25,
		// y-35, 50,50);
...
// });
    ...
    renderOrigin: function() {
        var dataOrigin = mixArray(staticOriginData, this.data.origin);
    ...
    _origin: function(obj, i) {
        ...
        // 百分比
        this.paper.text(obj.percentX, obj.percentY, obj.percent);
    ...
    _where: function(obj) {
        ...
        var nameObj = this.paper.text(obj.nameX, obj.nameY, obj.name+"\nPV: "+obj['pv']);
    ...
    /**
	 * 设置文本内容 svg 是tspan, vml 是textpath
	 */
    ...
    _show: function(index) {
        ...
        var obj = this.data.origin[index];
	 ...
	 *            endAngle 结束弧度
	 * @param {Object}
	 *            params 配置参数 {fill: '#d8d8d8', stroke: 'none'}
	 */
