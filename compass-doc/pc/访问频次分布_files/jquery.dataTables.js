...
(/** @lends <global> */function($, window, document, undefined) {
	...
	var DataTable = function( oInit )
	{
		 ...
		 * insert to head
		 */
		function _fnInfixData ( oSettings, aDataSupplied )
		{
			var oCol;
			...
			/* Add to the display array */
			oSettings.aiDisplayMaster.splice(0, 0, iRow );
		...
		function _fnBuildHead( oSettings )
		{
			...
			if ( iThs !== 0 )
			{
				...
				for ( i=0, iLen=oSettings.aoColumns.length ; i<iLen ; i++ )
				{
					...
					nTh.innerHTML = '<span class="sortIco">' + nTh.innerHTML + '</span>';
				}
			}
			else
			{
...
//				nTd.setAttribute( 'valign', "top" );
		...
		function _fnScrollDraw ( o )
		{
			var
				...
				ie67 = navigator.appName=="Microsoft Internet Explorer"&&(navigator.appVersion.indexOf("MSIE 6.0")>=0||navigator.appVersion.indexOf("MSIE 7.0")>=0);
		...
		function _fnCalculateColumnWidths ( oSettings )
		{
			...
			else
			{
				...
				for ( i=0 ; i<iColums ; i++ )
				{
					...
					{
					    //gaoch nThs[i-iCorrector].style.width = ""
						nThs[i-iCorrector]?nThs[i-iCorrector].style.width = "":"";
					}
					else
					{
						iCorrector++;
		...
		function _fnSort ( oSettings, bApplyClasses )
		{
				...
				(oSettings.aaSorting.length !== 0 || oSettings.aaSortingFixed !== null) )
			{
				...
				//create liyijun 2014_04_28
                var _containsValue = function(src,value){
                    for(var n in src){
                        if(src[n]==value){
                            return true;
                        }
				...
				{
					//create liyijun 2014_04_28
                    if(!!_oInit["custom"] && !!_oInit["custom"]["!sortingRows"]){
                        var _nSortingRows = _oInit["custom"]["!sortingRows"];
                        if(_containsValue(_nSortingRows,i)){
                            continue;
                        }
                    }
		...
		function _fnSortingClasses( oSettings )
		{
			...
			for ( i=0 ; i<iColumns ; i++ )
			{
				if ( oSettings.aoColumns[i].bSortable )
				{
					$(oSettings.aoColumns[i].nTh).find("span.sortIco").removeClass( oClasses.sSortAsc +" "+ oClasses.sSortDesc +
			...
			for ( i=0 ; i<oSettings.aoColumns.length ; i++ )
			{
				if ( oSettings.aoColumns[i].bSortable )
				{
					...
					$(oSettings.aoColumns[i].nTh).find("span.sortIco").addClass( sClass );
		...
		function _fnBindAction( n, oData, fn )
		{
			$(n)
				.bind( 'click.DT', oData, function (e) {
						fn(e);
						$(this).blur(); // Remove focus outline for mouse users
					} )
		 ...
		 * insert to head
		 */
		this.fnInfixData = function( mData, bRedraw )
		{
			if ( mData.length === 0 )
			{
				return [];
			...
			if ( typeof mData[0] === "object" && mData[0] !== null )
			{
				for ( var i=0 ; i<mData.length ; i++ )
				{
					iTest = _fnInfixData( oSettings, mData[i] );
					if ( iTest == -1 )
					{
						return aiReturn;
					}
			...
			{
				iTest = _fnInfixData( oSettings, mData );
				if ( iTest == -1 )
				{
					return aiReturn;
				}
		...
		"two_button": {
			...
			"fnInit": function ( oSettings, nPaging, fnCallbackDraw )
			{
				...
				var sAppend = (!oSettings.bJUI) ?
					'<a class="'+oSettings.oClasses.sPagePrevDisabled+'" tabindex="'+oSettings.iTabIndex+'" role="button"><b>< </b>'+oLang.sPrevious+'</a>'+
					'<a class="'+oSettings.oClasses.sPageNextDisabled+'" tabindex="'+oSettings.iTabIndex+'" role="button">'+oLang.sNext+'<b> ></b></a>'
					:
		...
		"full_numbers": {
			...
			"fnInit": function ( oSettings, nPaging, fnCallbackDraw )
			{
				...
				$(nPaging).append(
					...
					'<a  tabindex="'+oSettings.iTabIndex+'" class="'+oClasses.sPageButton+" "+oClasses.sPagePrevious+'"><b>< </b>'+oLang.sPrevious+'</a>'+
					'<span></span>'+
					'<a tabindex="'+oSettings.iTabIndex+'" class="'+oClasses.sPageButton+" "+oClasses.sPageNext+'">'+oLang.sNext+'<b> ></b></a>'+
